...
class ClosingCarsCommission(Document):
	def on_submit(self):
...
def getCB(beneficiary):
	mt = frappe.db.sql("""select name from `tabCommission Balance` where type = %s and beneficiary = %s;""",(beneficiary))
	return mt[0][0] if mt else ""


@frappe.whitelist(allow_guest=True)
def insert_data(from_date,to_date,beneficiary):
	query="select payment_method,transaction_date,name,grand_total,owner from `tabSales Order` where status = 'Completed' and is_car = 0 and delivery_car = '"+str(beneficiary)+"' and transaction_date between '"+str(from_date)+"' and '"+str(to_date)+"';"
	li=[]
...
def getCODorder(beneficiary,from_date,to_date):
	mt = frappe.db.sql("""select count(name) from `tabSales Order` where is_car = 0 and docstatus = 1 and 
			status = "Completed" 
			and delivery_car = %s and transaction_date between %s and %s;""",(beneficiary,from_date,to_date),as_list=1)
	return mt[0][0] if mt else 0.0

@frappe.whitelist(allow_guest=True)
def getDELDC(beneficiary):
	mt = frappe.db.sql("""select car_commission from `tabDelivery Car` where name = %s;""",(beneficiary),as_list=1)
	return mt[0][0] if mt else 0.0
